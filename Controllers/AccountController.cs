...
namespace Phosto.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var isBanned = db.Bans.FirstOrDefault(x=>(x.To>DateTime.Now && x.Username==model.Nick));
            if (isBanned == null)
            {
            ...
            {
                ModelState.AddModelError("", "You are banned until " + isBanned.To.ToString() + " becuse " + isBanned.Why + "!");
                return View(model);
            }
        }
        ...
        */
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Nick };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    var db = new ApplicationDbContext();
                    if (db.Users.Count()-1 == 0)
                    {
                        db.Roles.Add(new IdentityRole("Admin"));
                        db.Roles.Add(new IdentityRole("User"));
                        db.SaveChanges();
                        var userr = db.Users.Where(u => u.UserName.Equals(model.Nick)).FirstOrDefault();
